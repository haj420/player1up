...
const bumpers = [
  ...
  { x: W * 0.78, y: H * 0.33, r: 18, score: 500, force: 260 },
...
function reflectBallFromSegment(x1, y1, x2, y2) {
  const vx = x2 - x1;
  const vy = y2 - y1;
  const wx = ball.x - x1;
  const wy = ball.y - y1;
...
function drawDMD() {
  ...
  if (ball.inShooter) {
    ...
    ctx.fillText(`PLUNGER: ${Math.floor(plungerPower)}`, W / 2, 26);
  }
...
function drawFlipper(px, py, angle, isLeft) {
  ...
  ctx.save();
  ctx.strokeStyle = isLeft ? "#ff2bdconst plungerChargeRate = 480;
